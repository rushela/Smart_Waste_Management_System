...
const ErrorCard: React.FC<ErrorCardProps> = ({ paymentData }) => {
  const navigate = useNavigate();

  const formatDate = (dateString?: string) => {
    return new Date(dateString || new Date().toISOString()).toLocaleDateString('en-US', {
      weekday: 'long',
      year: 'numeric',
      month: 'long',
      day: 'numeric',
          ...
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-2 text-sm text-red-600">
                ...
                <span>Last attempt: {formatDate(paymentData?.date)}</span>
              </div>
            </div>
          </div>
        </div>
